...
- 9288 的 320×240 屏幕上，将 159×96 游戏 LCD 精确放大两倍为 318×192，
  显示在 `(1, 24)`，不裁切、不插值。
- 直接把 1 bpp 游戏画面转换到 9288 的 2 bpp framebuffer，不分配全屏
  中间缓冲。
...
- 两片 2 MiB ROM 使用 4 KiB bank 缓存，Flash 按游戏实际大小分配，适配
  9288 的 8 MiB SDRAM。
- 使用 ROS `get_tim()` 维护 16/17 ms 分数 deadline，以 60 Hz 推进模拟器
  时间；GUI 定时器只负责唤醒事件循环。
...
四个参数、R4 返回，并由加载器保留 R15。下面的 WSL/Linux 示例固定到本项目
验证过的工具链提交：
