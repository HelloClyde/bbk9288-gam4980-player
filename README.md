...
- 实验性的游戏加载时 AOT 可通过 `--game-load-aot` 开启。它在读取 `.gam` 时
  ...
  不生成可执行 RAM，也不绑定或打包某个游戏。《伏魔记》8000 帧剧情轨迹中有
  136 个块匹配，覆盖约 4.37% 的客机指令；9288 指令级模拟器中的核心计时约
  提升 1.08 倍。由于还需要真机验证，目前不在正式构建中默认开启。
...
`--no-aot`。下一版的游戏加载时 AOT 实验可传入 `--game-load-aot`；设计、
回退条件和基准结果见 [`docs/game-load-aot.md`](docs/game-load-aot.md)。
