...
实验构建的文件列表第一项是 `[设置]`：可以选择是否在加载游戏时扫描 AOT
模板，以及是否记录性能日志。设置保存在 `A:\gam4980\GAM4980.CFG`；开启
“性能调试”后，每次正常退出游戏都会向 `A:\gam4980\PERF.LOG` 追加一段记录。
...
- 正式构建默认启用受保护的 E.BIN 热点 AOT：将《伏魔记》开场轨迹中最热的
  46 个 65C02 基本块预先生成为 S1C33 C 超块。按从启动到退出的 8000 帧
  完整轨迹统计，覆盖全部客机指令约 72.0%、E.BIN 指令约 79.1%。
  已验证的相邻块会直接衔接，零页、固定 RAM 页和只读启动页会走专用快路径。
  每个块在首次进入时仍校验物理映射、相关 RAM bank 和 ROM 字节，不匹配便
  自动回退到解释器；不绑定、不打包任何 `.gam` 游戏。AOT 使 KF2 增加约
  24.3 KiB，`.bss` 仅增加 52 字节，不使用 JIT 缓冲区或额外堆内存。
- 实验性的游戏加载时 AOT 可通过 `--game-load-aot` 开启。它在读取 `.gam` 时
  识别七类常见 6502 基本块，把操作数绑定到 KF2 中预编译好的 S1C33 模板；
  不生成可执行 RAM，也不绑定或打包某个游戏。实验构建在游戏列表中提供持久化
  开关，并可按需记录加载耗时、客机帧、画面提交、ROM 读取、执行周期和热点
  物理 PC。《伏魔记》8000 帧剧情轨迹中有 136 个块匹配，覆盖约 4.37% 的客机
  指令；9288 指令级模拟器中的核心计时约提升 1.08 倍。由于还需要真机验证，
  目前不在正式构建中默认开启。
- 在同一台主机上用《伏魔记》运行 100,000 个客机帧，按完整剧情轨迹注入按键、
  交替测量 15 次取中位数，当前 AOT 核心相对同一份源码的纯解释器吞吐提升
  约 1.52 倍，执行时间减少约 34.3%。该数字不包含 9288 固件整屏传输和 GUI
  开销，不能直接等同于真机显示帧率。
...
`--no-aot`。游戏加载时 AOT 实验可传入 `--game-load-aot`；该构建同时加入
运行时设置和按需性能日志。设计、回退条件、日志字段和基准结果见
