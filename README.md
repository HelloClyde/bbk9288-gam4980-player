...
文件列表第一项是 `[设置]`：可以选择是否在加载游戏时扫描 AOT
...
- v1.5 默认启用游戏加载时 AOT 支持。它在读取 `.gam` 时
  识别七类常见 6502 基本块，把操作数绑定到 KF2 中预编译好的 S1C33 模板；
  不生成可执行 RAM，也不绑定或打包某个游戏。游戏列表中提供持久化
  开关，并可按需记录加载耗时、客机帧、画面提交、ROM 读取、执行周期和热点
  物理 PC。《伏魔记》8000 帧剧情轨迹中有 136 个块匹配，覆盖约 4.37% 的客机
  指令；9288 指令级模拟器中的核心计时约提升 1.08 倍。遇到不支持的代码形状、
  bank 映射变化或游戏修改 Flash 时会自动回退解释器。
